...
| Overlays              | `dialog.tsx`, `sheet.tsx`, Tailwind recipes, and residual named keyframes in `overlays.css`   | `dialog` and `sheet` metadata and examples                             | Focus management/restoration, dismissal, safe-area, responsive, and source-install evidence              | Already aligned                                                         |
