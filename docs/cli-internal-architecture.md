...
- deterministic generated bin: 56,716 to 65,472 bytes (`+8,756`, or `+15.4%`);
- package tarball: 20,243 to 23,356 bytes (`+3,113`, or `+15.4%`);
- unpacked package: 61,116 to 70,441 bytes (`+9,325`, or `+15.3%`);
...
this focused feature slice: the measured 3,113-byte increase buys two maintained deterministic
project profiles, atomic target creation, bounded JSON output, and clean package/build/browser
evidence. The 144-byte remaining headroom keeps future additions review-gated.
