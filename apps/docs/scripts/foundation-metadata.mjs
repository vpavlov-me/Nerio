...
function validateFoundationPages(pages) {
  ...
  for (const page of pages) {
    if (
      ...
      throw new Error(
        "Every foundation route must define path, label, title, description, and aliases values.",
      );
    }
    if (!page.path.startsWith("/docs/foundations/")) {
      throw new Error(`Foundation route ${page.path} must live under /docs/foundations/.`);
    }
    if (!page.label.trim() || !page.title.trim() || !page.description.trim()) {
      throw new Error(`Foundation route ${page.path} has empty discovery metadata.`);
    }
    ...
    if (titles.has(page.title)) throw new Error(`Duplicate foundation route title: ${page.title}.`);
    paths.add(page.path);
  ...
  for (const page of pages) {
    for (const alias of page.aliases) {
      if (typeof alias !== "string" || !alias.startsWith("/docs/foundations/")) {
        throw new Error(`Foundation alias for ${page.path} must live under /docs/foundations/.`);
      }
      if (paths.has(alias)) {
        throw new Error(`Foundation alias ${alias} competes with a canonical route.`);
      }
      if (aliases.has(alias)) throw new Error(`Duplicate foundation alias path: ${alias}.`);
      aliases.add(alias);
    }
  }
}
...
export function renderFoundationPagesModule(pages) {
  const navigationPages = pages.map(({ path, label }) => ({ path, label }));
  const aliases = pages.flatMap((page) =>
    page.aliases.map((path) => ({ path, destination: page.path })),
  );
  return `// Generated by apps/docs/scripts/validate-foundation-metadata.mjs. Do not edit.\n\nexport const foundationPages = ${JSON.stringify(navigationPages, null, 2)} as const;\n\nexport const foundationAliases = ${JSON.stringify(aliases, null, 2)} as const;\n`;
}

export function renderFoundationSearchPagesModule(pages) {
  return `// Generated by apps/docs/scripts/validate-foundation-metadata.mjs. Do not edit.\n\nexport const foundationPageMetadata = ${JSON.stringify(pages, null, 2)} as const;\n`;
}
...
export function foundationDiscoveryFailures({
  pages,
  llmsSource,
  routeExists,
  routeSource = () => null,
  implementedRoutes = [],
  aliasesUseGeneratedRedirects = true,
}) {
  const failures = [];
  const foundationsIndex =
    llmsSource.match(/(?:^|\n)The public Foundations index is[\s\S]*?(?=\n\n|$)/)?.[0] ?? "";
  const expectedPaths = pages.map((page) => page.path);
  const indexedPaths = [...foundationsIndex.matchAll(/`(\/docs\/foundations\/[^`]+)`/g)].map(
    (match) => match[1],
  );
  if (JSON.stringify(indexedPaths) !== JSON.stringify(expectedPaths)) {
    failures.push(
  ...
  for (const page of pages) {
    ...
    if (source) {
      for (const [snippet, label] of [
        ['<article className="doc-page">', "doc-page article"],
        ['<p className="doc-kicker">Foundation</p>', "Foundation kicker"],
        ["<h1", "page heading"],
        ['className="doc-lede"', "page lede"],
      ]) {
        if (!source.includes(snippet)) {
          failures.push(`${page.path}: canonical foundation page is missing its ${label}.`);
        }
      }
      if (!source.includes("<h2") && !source.includes('as="h2"')) {
        failures.push(
          `${page.path}: canonical foundation page needs section headings for the table of contents.`,
        );
      }
      if (/^\s*["']use client["'];/.test(source)) {
        failures.push(`${page.path}: canonical foundation page must remain server-rendered.`);
      }
      if (!source.includes(`getFoundationPage("${page.path}")`)) {
        failures.push(`${page.path}: page metadata must use the canonical foundation projection.`);
      }
    }
    for (const alias of page.aliases ?? []) {
      if (foundationsIndex.includes(`\`${alias}\``)) {
        failures.push(`${alias}: legacy alias must not compete in the Foundations index.`);
      }
    }
  }
  const canonicalPaths = new Set(expectedPaths);
  for (const route of implementedRoutes) {
    if (!canonicalPaths.has(route)) {
      failures.push(
        `${route}: implemented foundation route is missing canonical discovery metadata or an explicit redirect.`,
      );
    }
  }
  if (pages.some((page) => (page.aliases ?? []).length > 0) && !aliasesUseGeneratedRedirects) {
    failures.push("Foundation aliases must be wired through the generated redirect projection.");
  }
  return failures;
}
