...
async function verify() {
  ...
  try {
    ...
    if (
      !tooltipUsage.requiredTokens.includes("--n-overlay-glass-background") ||
      !tooltipUsage.requiredTokens.includes("--n-overlay-glass-foreground") ||
      !tooltipUsage.usage.includes("TooltipProvider") ||
      !tooltipUsage.accessibility.some((item) => item.includes("adjacent tooltip triggers"))
    ) {
      throw new Error("MCP Tooltip usage is missing glass overlay token or provider metadata.");
    }
