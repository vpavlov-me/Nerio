...
export const Switch = React.forwardRef<HTMLElement, SwitchProps>(function Switch(
  ...
  const control = (
    ...
    >
      <BaseSwitch.Thumb
        className="n-switch__thumb size-(--n-switch-thumb-size) flex-none translate-x-0 rounded-(--n-switch-thumb-radius) border-(length:--n-border-width-default) border-(--n-switch-thumb-border) bg-(--n-switch-thumb-background) transition-transform duration-(--n-motion-hover-duration) ease-(--n-motion-hover-easing) forced-colors:border-[CanvasText] motion-reduce:duration-(--n-duration-instant)"
        data-slot="thumb"
      />
    </BaseSwitch.Root>
  );
