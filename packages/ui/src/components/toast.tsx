...
const toastClasses =
  "n-toast flex max-w-(--n-toast-width) items-start gap-(--n-toast-gap) rounded-(--n-toast-radius) border-(length:--n-toast-border-width) border-(--n-toast-border) bg-(--n-toast-background) p-(--n-toast-padding) text-(--n-toast-foreground) shadow-(--n-toast-shadow) [-webkit-backdrop-filter:var(--n-overlay-surface-filter)] [backdrop-filter:var(--n-overlay-surface-filter)] [--n-button-background-secondary:var(--n-toast-control-background)] [--n-button-background-secondary-hover:var(--n-toast-control-background-hover)] [--n-button-background-secondary-active:var(--n-toast-control-background-active)] [--n-button-border-secondary:transparent] [--n-button-foreground-secondary:var(--n-toast-foreground)] [--n-button-background-ghost-hover:var(--n-toast-control-background-hover)] [--n-button-background-ghost-active:var(--n-toast-control-background-active)] [--n-button-foreground-ghost:var(--n-toast-foreground-muted)] data-[tone=info]:[--n-toast-status-color:var(--n-toast-status-info)] data-[tone=success]:[--n-toast-status-color:var(--n-toast-status-success)] data-[tone=warning]:[--n-toast-status-color:var(--n-toast-status-warning)] data-[tone=danger]:[--n-toast-status-color:var(--n-toast-status-danger)] [&_[data-slot=status-indicator]]:inline-flex [&_[data-slot=status-indicator]]:size-(--n-toast-status-indicator-size) [&_[data-slot=status-indicator]]:flex-none [&_[data-slot=status-indicator]]:items-center [&_[data-slot=status-indicator]]:justify-center [&_[data-slot=status-indicator]]:rounded-(--n-toast-status-indicator-radius) [&_[data-slot=status-indicator]]:text-(--n-toast-status-color) [&_[data-slot=status-indicator]_.n-icon]:size-(--n-icon-size-md) [&_[data-slot=title]]:m-0 [&_[data-slot=title]]:text-(length:--n-font-size-md) [&_[data-slot=title]]:leading-(--n-line-height-normal) [&_[data-slot=title]]:font-(--n-font-weight-regular) [&_[data-slot=title]]:text-(--n-toast-foreground) [&_[data-slot=description]]:m-0 [&_[data-slot=description]]:text-(length:--n-font-size-md) [&_[data-slot=description]]:leading-(--n-line-height-normal) [&_[data-slot=description]]:font-(--n-font-weight-regular) [&_[data-slot=description]]:text-(--n-toast-foreground-muted) [&_[data-slot=close]:hover:not(:disabled):not([data-disabled])]:text-(--n-toast-foreground) forced-colors:border-[CanvasText] forced-colors:[&_[data-slot=status-indicator]]:border forced-colors:[&_[data-slot=status-indicator]]:border-[ButtonText]";
const managedToastClasses =
  "n-toast--managed absolute right-0 bottom-0 z-[calc(var(--n-overlay-z-index)-var(--toast-index))] h-(--toast-managed-height) w-full origin-top touch-none select-none pointer-events-auto opacity-(--toast-managed-opacity) [--toast-managed-scale:max(0,calc(1-(var(--toast-index)*var(--n-toast-stack-scale-step))))] [--toast-managed-opacity:max(0.16,calc(1-(var(--toast-index)*var(--n-toast-stack-opacity-step))))] [--toast-managed-height:var(--toast-frontmost-height,var(--toast-height))] [--toast-managed-base-y:calc(var(--toast-index)*var(--n-toast-stack-offset)*-1)] [--toast-managed-dismiss-x:0px] [--toast-managed-dismiss-y:0px] [--toast-managed-enter-y:0px] [--toast-managed-x:calc(var(--toast-swipe-movement-x)+var(--toast-managed-dismiss-x))] [--toast-managed-y:calc(var(--toast-managed-base-y)+var(--toast-swipe-movement-y)+var(--toast-managed-enter-y)+var(--toast-managed-dismiss-y))] [transform:translate3d(var(--toast-managed-x),var(--toast-managed-y),0)_scale(var(--toast-managed-scale))] transition-[transform,opacity] duration-(--n-motion-reveal-duration) ease-(--n-motion-reveal-easing) data-expanded:[--toast-managed-height:var(--toast-height)] data-expanded:[--toast-managed-scale:1] data-expanded:[--toast-managed-opacity:1] data-expanded:[--toast-managed-base-y:calc((var(--toast-offset-y)*-1)-(var(--toast-index)*var(--n-toast-stack-gap)))] data-limited:pointer-events-none data-limited:[--toast-managed-opacity:var(--n-opacity-hidden)] data-starting-style:[--toast-managed-enter-y:var(--n-toast-enter-offset)] data-ending-style:[--toast-managed-opacity:var(--n-opacity-hidden)] data-ending-style:data-[swipe-direction=right]:[--toast-managed-dismiss-x:var(--n-toast-swipe-dismiss-distance)] data-ending-style:data-[swipe-direction=left]:[--toast-managed-dismiss-x:calc(-1*var(--n-toast-swipe-dismiss-distance))] data-ending-style:data-[swipe-direction=down]:[--toast-managed-dismiss-y:var(--n-toast-swipe-dismiss-distance)] data-ending-style:data-[swipe-direction=up]:[--toast-managed-dismiss-y:calc(-1*var(--n-toast-swipe-dismiss-distance))] after:absolute after:top-full after:left-0 after:h-[calc(var(--n-toast-stack-gap)+1px)] after:w-full after:content-[''] motion-reduce:duration-(--n-duration-instant) motion-reduce:data-starting-style:[--toast-managed-enter-y:0px] motion-reduce:data-ending-style:[--toast-managed-dismiss-x:0px] motion-reduce:data-ending-style:[--toast-managed-dismiss-y:0px]";
...
export function Toast({
  ...
  return (
    ...
    >
      <div
        className="n-toast__static-content grid min-w-0 flex-1 grid-cols-[var(--n-toast-status-indicator-size)_minmax(0,1fr)] content-start items-center gap-x-(--n-space-2) gap-y-(--n-toast-copy-gap) [overflow-wrap:anywhere]"
        data-slot="content"
      >
        <ToastIndicator tone={tone} />
        <strong data-slot="title">{title}</strong>
        {description ? (
          <span className="col-start-2" data-slot="description">
            {description}
          </span>
        ) : null}
      </div>
...
function ToastList({
  ...
  return manager.toasts.map((toast) => {
    ...
    return (
      ...
      >
        <BaseToast.Content
          className="n-toast__content flex flex-1 items-start gap-(--n-toast-content-gap) max-[30rem]:flex-wrap"
          data-slot="content"
        >
          <div className="n-toast__copy grid min-w-0 flex-1 grid-cols-[var(--n-toast-status-indicator-size)_minmax(0,1fr)] content-start items-center gap-x-(--n-space-2) gap-y-(--n-space-1) [overflow-wrap:anywhere] max-[30rem]:basis-[calc(100%-var(--n-toast-status-indicator-size)-var(--n-toast-content-gap))]">
            <ToastIndicator tone={tone} />
            <BaseToast.Title data-slot="title" />
            <BaseToast.Description className="col-start-2" data-slot="description" />
          </div>
          {action ? (
            <Button
              render={
