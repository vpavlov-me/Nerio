...
const fileInputBaseClasses =
  "n-file-input box-border w-full min-w-0 cursor-pointer rounded-(--n-input-radius) border-(length:--n-input-border-width) border-(--n-input-border) bg-(--n-input-background) pe-(--n-input-padding-inline) text-(length:--n-input-font-size) font-(--n-input-font-weight) text-(--n-input-foreground) file:me-(--n-file-input-button-gap) file:w-(--n-file-input-button-size) file:cursor-pointer file:border-0 file:bg-(--n-file-input-button-background) file:px-0 file:text-transparent [&:hover:not(:disabled)]:border-(--n-input-border-hover) [&:hover:not(:disabled)]:bg-(--n-input-background-hover) hover:file:bg-(--n-file-input-button-background-hover) focus-visible:border-(--n-input-border-focus) focus-visible:outline-0 focus-visible:shadow-(--n-focus-ring) disabled:cursor-not-allowed disabled:bg-(--n-input-disabled-background) disabled:text-(--n-input-disabled-foreground) disabled:opacity-(--n-input-disabled-opacity) disabled:file:cursor-not-allowed disabled:file:bg-(--n-file-input-button-disabled-background) data-invalid:border-(--n-input-border-danger) aria-invalid:border-(--n-input-border-danger) forced-colors:border-[CanvasText] forced-colors:file:bg-[ButtonFace] forced-colors:file:text-transparent forced-colors:data-invalid:border-[Mark]";
