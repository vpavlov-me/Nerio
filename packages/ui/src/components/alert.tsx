...
const alertClasses =
  "n-alert grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-(--n-alert-gap) rounded-(--n-alert-radius) border-(length:--n-alert-border-width) border-(--n-alert-border) bg-(--n-alert-background) p-(--n-alert-padding) text-(--n-color-text-secondary) shadow-(--n-alert-shadow) [&:not(:has(.n-alert__icon))]:grid-cols-[minmax(0,1fr)_auto] [&:not(:has(.n-alert__action))]:grid-cols-[auto_minmax(0,1fr)] [&:not(:has(.n-alert__icon)):not(:has(.n-alert__action))]:grid-cols-1 [&:not(:has(.n-alert__description))]:items-center [&:not(:has(.n-alert__description))_.n-alert__action]:self-center data-[tone=info]:[--n-alert-icon-color:var(--n-color-status-info)] data-[tone=info]:[--n-alert-title-color:var(--n-color-status-info)] data-[tone=success]:[--n-alert-icon-color:var(--n-color-status-success)] data-[tone=success]:[--n-alert-title-color:var(--n-color-status-success)] data-[tone=warning]:[--n-alert-icon-color:var(--n-color-status-warning)] data-[tone=warning]:[--n-alert-title-color:var(--n-color-status-warning)] data-[tone=danger]:[--n-alert-icon-color:var(--n-color-status-danger)] data-[tone=danger]:[--n-alert-title-color:var(--n-color-status-danger)]";
