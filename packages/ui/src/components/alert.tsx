...
const alertClasses =
  "n-alert grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-(--n-alert-gap) rounded-(--n-alert-radius) border-(length:--n-alert-border-width) border-(--n-alert-border) bg-(--n-alert-background) p-(--n-alert-padding) text-(--n-color-text-secondary) shadow-(--n-alert-shadow) [&:not(:has(>[data-slot=icon]))]:grid-cols-[minmax(0,1fr)_auto] [&:not(:has(>[data-slot=action]))]:grid-cols-[auto_minmax(0,1fr)] [&:not(:has(>[data-slot=icon])):not(:has(>[data-slot=action]))]:grid-cols-1 [&:not(:has(>[data-slot=content]>[data-slot=description]))]:items-center [&:not(:has(>[data-slot=content]>[data-slot=description]))>[data-slot=action]]:self-center data-[tone=info]:[--n-alert-icon-color:var(--n-color-status-info)] data-[tone=success]:[--n-alert-icon-color:var(--n-color-status-success)] data-[tone=warning]:[--n-alert-icon-color:var(--n-color-status-warning)] data-[tone=danger]:[--n-alert-icon-color:var(--n-color-status-danger)]";
