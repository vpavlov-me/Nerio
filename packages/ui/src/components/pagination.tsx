...
const paginationControlClasses =
  "n-pagination__control inline-flex min-h-(--n-pagination-item-size) min-w-(--n-pagination-item-size) items-center justify-center rounded-(--n-pagination-radius) border-(length:--n-pagination-border-width) border-(--n-pagination-border) bg-(--n-pagination-background) px-(--n-pagination-control-padding-inline) font-(--n-font-weight-medium) text-(--n-pagination-foreground) shadow-(--n-pagination-shadow) no-underline hover:bg-(--n-pagination-background-hover) hover:text-(--n-pagination-foreground-hover) focus-visible:rounded-(--n-radius-sm) focus-visible:outline-0 focus-visible:shadow-(--n-focus-ring) data-current:border-(--n-pagination-border-current) data-current:bg-(--n-pagination-background-current) data-current:font-(--n-font-weight-semibold) data-current:text-(--n-pagination-foreground-current) data-current:shadow-(--n-pagination-shadow-current) data-disabled:cursor-not-allowed data-disabled:text-(--n-color-text-disabled) data-disabled:opacity-(--n-opacity-muted) [&:is(button)]:cursor-pointer [&:is(button)]:appearance-none [&:is(button)]:font-inherit forced-colors:border-[CanvasText] forced-colors:data-current:outline-(length:--n-focus-ring-inner-width) forced-colors:data-current:outline-[Highlight] forced-colors:focus-visible:outline-(length:--n-focus-ring-inner-width) forced-colors:focus-visible:outline-offset-(--n-focus-ring-inner-width) forced-colors:focus-visible:outline-[Highlight]";
