...
import { Button, Dialog, IconButton, Select, Tabs, Toast, ToastProvider, ToastViewport, useToastManager } from "../src/client";
...
describe("Core static contracts", () => {
  it("keeps Progress ARIA on the progressbar and normalizes unsafe values", () => {
    ...
    expect(progressbar.querySelector("[data-slot=indicator]")).toHaveStyle("--n-progress-value: 100%");
  ...
  it.each([[0, 0], [50, 50], [100, 100], [-4, 0], [104, 100]])(
    "clamps Progress value %s to %s",
    (value, expected) => {
      ...
      expect(screen.getByRole("progressbar")).toHaveAttribute("aria-valuetext", `${expected}% complete`);
    },
  );
  ...
  it("allows deliberate Card and EmptyState heading semantics", () => {
    render(
      <>
        <Card as="article"><CardTitle as="h2">Overview</CardTitle></Card>
  ...
  it("supports Avatar alt overrides, one-word names, and every size", () => {
    const { rerender } = render(<Avatar name="Maya" size="sm" src="/maya.png" alt="Profile photo" />);
    expect(screen.getByRole("img", { name: "Profile photo" }).closest("span")).toHaveAttribute("data-size", "sm");
  ...
  it("keeps ordered lists visibly ordered and supports stable item IDs", () => {
    render(<List ordered items={[{ id: "one", title: "First" }, { id: "two", title: "Second" }]} />);
  ...
  it("allows router pagination rendering and preserves labels on disabled boundaries", () => {
    render(<Pagination previousAriaLabel="Older" nextAriaLabel="Newer" pages={[{ key: "router", label: "Router", href: "/router", render: <a data-router-link="" /> }]} />);
  ...
  it("only creates a keyboard scroll stop for an explicit TableContainer", () => {
    const { rerender } = render(<TableContainer><Table><TableBody><TableRow><TableCell>A</TableCell></TableRow></TableBody></Table></TableContainer>);
    expect(screen.getByText("A").closest("div")).not.toHaveAttribute("tabindex");
    rerender(<TableContainer focusable label="Projects"><Table><TableBody><TableRow><TableCell>A</TableCell></TableRow></TableBody></Table></TableContainer>);
  ...
  it("uses semantic names and native associations @a11y", async () => {
    const { container } = render(
      <>
        <Alert tone="warning" title="Review needed">A required field is missing.</Alert>
        <Badge tone="success">Ready</Badge>
        <Spinner label="Saving" />
        <Breadcrumbs items={[{ label: "Docs", href: "/docs" }, { label: "Buttons" }]} />
        <Field label="Email" description="We only use this for updates." message="Required" invalid><Input /></Field>
        <FormGroup title="Notifications" description="Choose channels."><input type="checkbox" aria-label="Email" /></FormGroup>
      </>,
    );
  ...
  it("generates Field associations, preserves consumer IDs, and rejects multiple controls", () => {
    const { rerender } = render(<Field label="Email" description="Updates only" message="Required" invalid><Input /></Field>);
    ...
    rerender(<Field label="Project"><Input id="project-name" /></Field>);
    expect(screen.getByRole("textbox", { name: "Project" })).toHaveAttribute("id", "project-name");
    expect(() => render(<Field label="Invalid"><Input /><Input /></Field>)).toThrow("exactly one");
  });
  ...
  it("keeps default Card and EmptyState semantics and forwards refs", () => {
    ...
    render(<><Card ref={cardRef}>Card</Card><EmptyState ref={emptyRef} title="Empty" description="Nothing here" /></>);
  ...
  it("renders managed Toast actions and localized dismiss controls", async () => {
    ...
    function Trigger() {
      ...
      return <button onClick={() => manager.add({ title: "Saved", data: { tone: "success", action: { label: "Undo", onClick: action } } })}>Show</button>;
    }
    render(<ToastProvider><Trigger /><ToastViewport dismissLabel="Close notification" /></ToastProvider>);
    await user.click(screen.getByRole("button", { name: "Show" }));
    await user.click(await screen.findByRole("button", { name: "Undo" }));
    expect(action).toHaveBeenCalledOnce();
    expect(screen.getByRole("button", { name: "Close notification" })).toHaveAttribute("data-slot", "close");
  });
});
...
describe("Core interactive action contracts", () => {
  ...
  it("supports uncontrolled Dialog open, Escape close, and focus restoration", async () => {
    const user = userEvent.setup();
    render(<Dialog trigger="Open settings" title="Settings">Dialog body</Dialog>);
  ...
  it("supports controlled Select values, labels, descriptions, and invalid state", () => {
    render(<Select label="Status" description="Workflow state" message="Required" invalid value="draft" options={[{ label: "Draft", value: "draft" }, { label: "Published", value: "published" }]} />);
  ...
  it("uses the first enabled Tab, skips disabled tabs, and calls controlled state handlers", async () => {
    ...
    render(<Tabs onChange={onChange} variant="segmented" tabs={[{ label: "Disabled", value: "disabled", content: "No", disabled: true }, { label: "Overview", value: "overview", content: "Overview panel" }, { label: "Activity", value: "activity", content: "Activity panel" }]} />);
